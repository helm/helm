...
func (i *OCIInstaller) Install() error {
	...
	metadata, err := plugin.ExtractPluginMetadataFromReader(bytes.NewReader(i.pluginData))
	if err != nil {
...
func (i *OCIInstaller) GetVerificationData() (archiveData, provData []byte, filename string, err error) {
	...
	// Download prov data once and cache it
	if i.provData == nil {
		provSource := i.Source + ".prov"
		provDataBuffer, err := i.getter.Get(provSource)
		if err != nil {
			return nil, nil, "", fmt.Errorf("failed to pull provenance from %s: %w", provSource, err)
		}
