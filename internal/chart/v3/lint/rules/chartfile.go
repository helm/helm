...
// validChartName is a regular expression for chart names.
//
// Chart names must follow DNS-1123 label format and must consist of lower
...
func validateChartName(cf *chart.Metadata) error {
	...
	if !validChartName.MatchString(cf.Name) {
		return fmt.Errorf("chart name %q does not follow DNS-1123 label format (must consist of lower case alphanumeric characters or '-', and must start and end with an alphanumeric character)", cf.Name)
	}
	if len(cf.Name) > 63 {
		return fmt.Errorf("chart name %q is too long (maximum is 63 characters)", cf.Name)
	}
	return nil
}
