...
func (suite *RegistryClientTestSuite) Test_1_Push() {
	...
	// Load a test chart
	chartData, err := os.ReadFile("../../../pkg/repo/repotest/testdata/examplechart-0.1.0.tgz")
	...
	// basic push, good ref
	chartData, err = os.ReadFile("../../../pkg/downloader/testdata/local-subchart-0.1.0.tgz")
	suite.Nil(err, "no error loading test chart")
	meta, err = extractChartMeta(chartData)
	suite.Nil(err, "no error extracting chart meta")
	...
	chartData, err = os.ReadFile("../../../pkg/downloader/testdata/signtest-0.1.0.tgz")
	suite.Nil(err, "no error loading test chart")
	meta, err = extractChartMeta(chartData)
	suite.Nil(err, "no error extracting chart meta")

	// Load prov file
	provData, err := os.ReadFile("../../../pkg/downloader/testdata/signtest-0.1.0.tgz.prov")
