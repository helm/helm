...
`
	...
	// --- Authoritative name-based lookup: error cases ---

	// Repo name provided but not in repos map: falls through to URL scan
	// (backward compatibility for repos generated by ensureMissingRepos).
	_, _, _, _, _, _, _, _, err = m.findChartURL(name, version, repoURL, "nonexistent", repos)
	if err != nil {
		t.Errorf("Expected fallback to URL scan for repo not in map, got error: %v", err)
	}

	// Repo name valid but chart not found in that specific repo: must error,
	// must not fall through to URL-based scanning.
	_, _, _, _, _, _, _, _, err = m.findChartURL("nonexistent-chart", version, repoURL, "repo-alias-1", repos)
	if err == nil {
		t.Error("Expected error for nonexistent chart in named repo, got nil")
	}

	// Repo name valid, chart exists, but version not found: must error,
	// must not fall through to URL-based scanning.
	_, _, _, _, _, _, _, _, err = m.findChartURL(name, "99.99.99", repoURL, "repo-alias-1", repos)
	if err == nil {
		t.Error("Expected error for nonexistent version in named repo, got nil")
	}
}
