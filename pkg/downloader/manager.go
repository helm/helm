...
func (m *Manager) findChartURL(name, version, repoURL, repoName string, repos map[string]*repo.ChartRepository) (url, username, password string, insecureSkipTLSVerify, passCredentialsAll bool, caFile, certFile, keyFile string, err error) {
	...
	// When a repo name is provided and exists in the repos map, the lookup
	// is authoritative: if the chart is not found or URL resolution fails,
	// an error is returned rather than silently falling back to URL-based
	// scanning. If the named repo is not in the repos map (e.g. repos from
	// ensureMissingRepos are not persisted to repositories.yaml), we fall
	// through to URL-based scanning for backward compatibility.
	if repoName != "" {
		if cr, ok := repos[repoName]; ok {
			var entry repo.ChartVersions
			entry, err = findEntryByName(name, cr)
			if err != nil {
				err = fmt.Errorf("chart %q not found in repository %q", name, repoName)
				return
			}
			...
			if err != nil {
				err = fmt.Errorf("chart %q version %q not found in repository %q", name, version, repoName)
				return
			}
		...
		// Repo name provided but not in the repos map.
		// Fall through to URL-based scanning for backward compatibility
		// (handles repos generated by ensureMissingRepos).
	}

	// Fall back to scanning repos by URL for backward compatibility
	// (only when no repo name was specified, or the named repo is not in
	// the repos map).
