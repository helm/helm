...
// Resolver resolves libraries/dependencies from semantic version ranges to a particular version.
...
// Resolve resolves libraries/dependencies and returns a lock file with the resolution.
func (r *Resolver) Resolve(reqs []*chart.Dependency, repoNames map[string]string, d string, isLib bool) (*chart.Lock, error) {

	// Now we clone the libraries/dependencies, locking as we go.
	...
	for i, d := range reqs {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "dependency/library %q has an invalid version/constraint format", d.Name)
		}
...
// HashReq generates a hash of the libraries/dependencies.
//
...
// "file://" in repository of libraries/dependencies
