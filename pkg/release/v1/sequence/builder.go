...
func (b *builder) appendResourceGroupBatches(chartPath string, depth int, manifests []releaseutil.Manifest) error {
	...
	for _, manifest := range manifests {
		...
		hasSuccess := readinessAnnotationSet(annotations, releaseutil.AnnotationReadinessSuccess)
		hasFailure := readinessAnnotationSet(annotations, releaseutil.AnnotationReadinessFailure)
...
func hasCustomReadiness(manifests []releaseutil.Manifest) bool {
	for _, manifest := range manifests {
		...
		if readinessAnnotationSet(annotations, releaseutil.AnnotationReadinessSuccess) && readinessAnnotationSet(annotations, releaseutil.AnnotationReadinessFailure) {
			return true
		}
	}
	return false
...
// readinessAnnotationSet reports whether a readiness annotation carries a
// usable value. Whitespace-only values count as unset, matching
// parseReadinessExpressions in pkg/kube, which trims and discards empty
// expressions at evaluation time.
func readinessAnnotationSet(annotations map[string]string, key string) bool {
