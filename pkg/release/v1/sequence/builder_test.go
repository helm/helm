...
func TestBuild_WhitespaceReadinessAnnotationsAreUnset(t *testing.T) {
	t.Parallel()

	// Whitespace-only values on both annotations: no custom readiness and no
	// partial-readiness warning.
	groupA := groupAnnotations("a")
	groupA[releaseutil.AnnotationReadinessSuccess] = "   "
	groupA[releaseutil.AnnotationReadinessFailure] = ""
	// Whitespace-only success with a real failure expression is partial.
	groupB := groupAnnotations("b", "a")
	groupB[releaseutil.AnnotationReadinessSuccess] = " "
	groupB[releaseutil.AnnotationReadinessFailure] = `{.status.failed} == true`
	manifests := []releaseutil.Manifest{
		makeManifest("a", "parent/templates/a.yaml", groupA),
		makeManifest("b", "parent/templates/b.yaml", groupB),
	}
	...
	assert.False(t, plan.Batches[0].HasCustomReadiness)
	assert.False(t, plan.Batches[1].HasCustomReadiness)
	require.Len(t, plan.Warnings, 1)
	assertWarningContains(t, plan, "parent", "only one of", `"b"`)
	assertPlanComplete(t, plan, manifests)
}
