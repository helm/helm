...
// most git commands will work with an https remote url, but the 'git archive' command
// requires the 'https:' protocol be replaced with 'git:'
// No change is needed for SSH urls.
func setURLProtocolForGitArchive(gitURL string) string {
	if strings.HasPrefix(gitURL, "https:") {
		return strings.Replace(gitURL, "https:", "git:", 1)
	}
	return gitURL
}
