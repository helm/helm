...
// log returns the configured logger or a discard logger if none is set.
func (c *ReadyChecker) log() *slog.Logger {
	if c.logger != nil {
		return c.logger
	}
...
// WithLogger returns a ReadyCheckerOption that sets the logger.
func WithLogger(logger *slog.Logger) ReadyCheckerOption {
	return func(c *ReadyChecker) {
		c.logger = logger
	}
}
...
func (c *ReadyChecker) jobReady(job *batchv1.Job) (bool, error) {
	...
	c.log().Debug("Job is completed", "namespace", job.GetNamespace(), "name", job.GetName())
	return true, nil
}
...
func (c *ReadyChecker) serviceReady(s *corev1.Service) bool {
	...
	c.log().Debug("Service is ready", "namespace", s.GetNamespace(), "name", s.GetName(), "clusterIP", s.Spec.ClusterIP, "externalIPs", s.Spec.ExternalIPs)
	return true
}
...
func (c *ReadyChecker) volumeReady(v *corev1.PersistentVolumeClaim) bool {
	...
	c.log().Debug("PersistentVolumeClaim is bound", "namespace", v.GetNamespace(), "name", v.GetName(), "phase", v.Status.Phase)
	return true
}
...
func (c *ReadyChecker) deploymentReady(rs *appsv1.ReplicaSet, dep *appsv1.Deployment) bool {
	...
	if dep.Status.ObservedGeneration != dep.Generation {
		c.log().Debug("Deployment is not ready, observedGeneration does not match spec generation", "namespace", dep.GetNamespace(), "name", dep.GetName(), "actualGeneration", dep.Status.ObservedGeneration, "expectedGeneration", dep.Generation)
		return false
	}
	...
	c.log().Debug("Deployment is ready", "namespace", dep.GetNamespace(), "name", dep.GetName(), "readyPods", rs.Status.ReadyReplicas, "totalPods", expectedReady)
	return true
}
...
func (c *ReadyChecker) daemonSetReady(ds *appsv1.DaemonSet) bool {
	...
	if ds.Status.ObservedGeneration != ds.Generation {
		c.log().Debug("DaemonSet is not ready, observedGeneration does not match spec generation", "namespace", ds.GetNamespace(), "name", ds.GetName(), "observedGeneration", ds.Status.ObservedGeneration, "expectedGeneration", ds.Generation)
		return false
	}
	...
	c.log().Debug("DaemonSet is ready", "namespace", ds.GetNamespace(), "name", ds.GetName(), "readyPods", ds.Status.NumberReady, "totalPods", expectedReady)
	return true
}
...
func (c *ReadyChecker) statefulSetReady(sts *appsv1.StatefulSet) bool {
	...
	if sts.Status.ObservedGeneration != sts.Generation {
		c.log().Debug("StatefulSet is not ready, observedGeneration doest not match spec generation", "namespace", sts.GetNamespace(), "name", sts.GetName(), "actualGeneration", sts.Status.ObservedGeneration, "expectedGeneration", sts.Generation)
		return false
	}
...
func (c *ReadyChecker) replicationControllerReady(rc *corev1.ReplicationController) bool {
	...
	if rc.Status.ObservedGeneration != rc.Generation {
		c.log().Debug("ReplicationController is not ready, observedGeneration doest not match spec generation", "namespace", rc.GetNamespace(), "name", rc.GetName(), "actualGeneration", rc.Status.ObservedGeneration, "expectedGeneration", rc.Generation)
		return false
	}
	return true
}
...
func (c *ReadyChecker) replicaSetReady(rs *appsv1.ReplicaSet) bool {
	...
	if rs.Status.ObservedGeneration != rs.Generation {
		c.log().Debug("ReplicaSet is not ready, observedGeneration doest not match spec generation", "namespace", rs.GetNamespace(), "name", rs.GetName(), "actualGeneration", rs.Status.ObservedGeneration, "expectedGeneration", rs.Generation)
		return false
	}
	return true
}
