...
func TestBackwardCompat_MixedAnnotationsUnsequencedLast(t *testing.T) {
	...
	// readiness is not waited on. The unsequenced plain ConfigMaps are not
	// part of the resource-group DAG and are waited on as before.
	require.Len(t, client.waitCalls, 2)
	assert.Equal(t, []string{"ConfigMap/database"}, client.waitCalls[0])
	assert.ElementsMatch(t, client.createCalls[2], client.waitCalls[1])
	assert.Empty(t, buf.String())
	assert.Equal(t, rcommon.StatusDeployed, rel.Info.Status)
}
