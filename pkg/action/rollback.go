...
func (r *Rollback) performRollback(currentRelease, targetRelease *release.Release) (*release.Release, error) {
	...
	if err != nil {
		...
		if r.CleanupOnFail {
			...
			if errs != nil {
				return targetRelease, fmt.Errorf("rollback failed: %w: unable to clean up resources: %w", err, errors.Join(errs...))
			}
	...
	if r.Wait {
		if r.WaitForJobs {
			...
			if err := r.cfg.KubeClient.Wait(target, r.Timeout); err != nil {
				...
				r.cfg.recordRelease(currentRelease)
				r.cfg.recordRelease(targetRelease)
				return targetRelease, fmt.Errorf("release %s failed", targetRelease.Name)
			}
		}
	}
