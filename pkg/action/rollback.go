...
func (r *Rollback) performRollback(currentRelease, targetRelease *release.Release) (*release.Release, error) {
	...
	// pre-rollback hooks
	if !r.DisableHooks {
		if err := r.cfg.execHook(targetRelease, release.HookPreRollback, nil, r.Timeout); err != nil {
	...
	// post-rollback hooks
	if !r.DisableHooks {
		if err := r.cfg.execHook(targetRelease, release.HookPostRollback, nil, r.Timeout); err != nil {
			return targetRelease, err
		}
	}
