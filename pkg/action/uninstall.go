...
func (u *Uninstall) deleteReleaseSequenced(rel *release.Release, waiter kube.Waiter) (kube.ResourceList, string, []error, bool) {
	...
	for _, batch := range plan.Reverse().Batches {
		deleted, skipped, errs := u.deleteManifestBatch(batch.Manifests(), waiter, deadline, rel.Name, rel.Namespace)
...
func (u *Uninstall) deleteManifestBatch(manifests []releaseutil.Manifest, waiter kube.Waiter, deadline time.Time, releaseName, releaseNamespace string) (kube.ResourceList, string, []error) {
	// Fail fast when the overall uninstall deadline has already passed, so a
	// later batch never issues deletes it has no time left to wait on.
	if !deadline.IsZero() && time.Until(deadline) <= 0 {
		return nil, "", []error{fmt.Errorf("uninstall timed out after %s before all batches were deleted", u.Timeout)}
	}
	...
	ownedResources, skipped, err := u.verifyOwnedForDelete(resources, releaseName, releaseNamespace)
	if err != nil {
		return nil, "", []error{err}
	}
	if len(ownedResources) == 0 {
	...
	_, errs := u.cfg.KubeClient.Delete(ownedResources, parseCascadingFlag(u.DeletionPropagation, u.cfg.Logger()))
	if len(errs) > 0 {
		return ownedResources, skipped, errs
	}
