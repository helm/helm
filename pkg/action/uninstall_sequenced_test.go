...
func TestUninstall_Sequenced_SkipsUnownedResources(t *testing.T) {
	...
	// The skipped summary must surface in the response Info, exactly as the
	// non-sequenced uninstall path reports it.
	require.NotNil(t, res)
	is.Contains(res.Info, "not owned by this release")
	is.Contains(res.Info, "unowned-deploy")
}

func TestUninstall_Sequenced_DeadlineExceededFailsFast(t *testing.T) {
	uninstall := NewUninstall(actionConfigFixture(t))
	uninstall.Timeout = time.Minute

	deleted, skipped, errs := uninstall.deleteManifestBatch(nil, nil, time.Now().Add(-time.Second), "rel", "ns")
