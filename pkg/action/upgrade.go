...
func (u *Upgrade) failRelease(rel *release.Release, created kube.ResourceList, err error) (*release.Release, error) {
	...
	if u.CleanupOnFail && len(created) > 0 {
		...
		if errs != nil {
			return rel, fmt.Errorf("upgrade failed: %w: unable to clean up resources: %w", err, errors.Join(errs...))
		}
	...
	if u.Atomic {
		...
		if herr != nil {
			return rel, fmt.Errorf("unable to find last successful release %w: upgrade failed: %w: ", herr, err)
		}
		...
		if len(filteredHistory) == 0 {
			return rel, fmt.Errorf("unable to find a previously successful release when attempting to rollback: upgrade failed: %w", err)
		}
		...
		if rollErr := rollin.Run(rel.Name); rollErr != nil {
			return rel, fmt.Errorf("rollback failed: %w: upgrade failed: %w", rollErr, err)
		}
