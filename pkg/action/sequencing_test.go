...
type recordingKubeClient struct {
	...
	deleteCalls          [][]string
	waiterStrategies     []kube.WaitStrategy
	operations           []string
	waitCallCount        int
	waitErrorOnCall      int
	waitError            error
	onBuild              func()
	// buildRESTClient, when set, attaches a fake REST client to every
	// resource.Info produced by Build — needed by tests whose action path
	// performs live existence/ownership checks (a real Build always attaches
	// a client; most recording tests never hit those paths and omit it).
	buildRESTClient func(u *unstructured.Unstructured) resource.RESTClient
	// createdObjects tracks "Kind/name" ids that Create has been called for,
	// letting stateful fakes answer GETs like a live cluster would.
...
func newRecordingKubeClient() *recordingKubeClient {
	return &recordingKubeClient{
		...
		// A real Build always attaches a REST client to each Info; mirror
		// that with a fake that reports 404 so existence/conflict checks in
		// the install/upgrade prologue exercise the real "resource does not
		// exist yet" path. Delete-path tests override this (see
		// withoutBuildClients) because their ownership checks would read a
		// 404 as "already deleted".
		buildRESTClient: func(u *unstructured.Unstructured) resource.RESTClient {
			return fakeClientWith(http.StatusNotFound, u.GroupVersionKind().GroupVersion(), "")
		},
		createdObjects: make(map[string]bool),
	}
}

// withoutBuildClients makes Build produce Infos with no REST client, for
// delete-path tests that rely on the nil-client ownership-verification guard
// (verifyOwnershipBeforeDelete treats a fetch 404 as "already deleted", which
// would wrongly skip deletions for these fixtures).
func (c *recordingKubeClient) withoutBuildClients() *recordingKubeClient {
	c.buildRESTClient = nil
	return c
}

// statefulOwnedRESTClient mimics a live cluster across an install-then-delete
// flow: GETs report 404 until Create has been called for the object, and
// afterwards return it labeled/annotated as owned by the given release, so
// post-failure rollback deletion passes ownership checks even though the
// delete path re-Builds fresh resource.Infos from the stored manifest.
func statefulOwnedRESTClient(c *recordingKubeClient, releaseName, releaseNamespace string) func(u *unstructured.Unstructured) resource.RESTClient {
	return func(u *unstructured.Unstructured) resource.RESTClient {
		gvk := u.GroupVersionKind()
		id := fmt.Sprintf("%s/%s", gvk.Kind, u.GetName())
		ownedBody := fmt.Sprintf(
			`{"apiVersion":%q,"kind":%q,"metadata":{"name":%q,"labels":{%q:%q},"annotations":{%q:%q,%q:%q}}}`,
			gvk.GroupVersion().String(), gvk.Kind, u.GetName(),
		...
		return &fake.RESTClient{
			...
			Client: fake.CreateHTTPClient(func(_ *http.Request) (*http.Response, error) {
				...
				if c.createdObjects[id] {
					code, body = http.StatusOK, ownedBody
				}
				return &http.Response{StatusCode: code, Header: header, Body: stringBody(body)}, nil
			}),
		}
	}
}
...
func (c *recordingKubeClient) Build(reader io.Reader, _ bool) (kube.ResourceList, error) {
	...
	for {
		...
		if c.buildRESTClient != nil {
			info.Client = c.buildRESTClient(u)
		}
...
func (c *recordingKubeClient) GetWaiterWithOptions(ws kube.WaitStrategy, _ ...kube.WaitOption) (kube.Waiter, error) {
	c.waiterStrategies = append(c.waiterStrategies, ws)
	if ws == kube.HookOnlyStrategy {
		// Mirror the real hookOnlyWaiter: general waits are no-ops, only
		// hook watches are real (pkg/kube/statuswait.go:297-315).
		return &recordingHookOnlyWaiter{client: c}, nil
	}
	return &recordingKubeWaiter{client: c}, nil
}

type recordingHookOnlyWaiter struct {
	client *recordingKubeClient
}

func (w *recordingHookOnlyWaiter) Wait(_ kube.ResourceList, _ time.Duration) error {
	return nil
}

func (w *recordingHookOnlyWaiter) WaitWithJobs(_ kube.ResourceList, _ time.Duration) error {
	return nil
}

func (w *recordingHookOnlyWaiter) WaitForDelete(_ kube.ResourceList, _ time.Duration) error {
	return nil
}

func (w *recordingHookOnlyWaiter) WatchUntilReady(resources kube.ResourceList, d time.Duration) error {
	return (&recordingKubeWaiter{client: w.client}).WatchUntilReady(resources, d)
}
...
// TestInstall_Sequenced_LeafGroupsWaited locks D1: ordered is a --wait
// flavor, so terminal groups are gated too. This makes readiness-failure
// detection and --atomic work for them.
func TestInstall_Sequenced_LeafGroupsWaited(t *testing.T) {
	client := newRecordingKubeClient()
	install := newSequencedInstallAction(t, client)
	...
	assert.Equal(t, [][]string{{"ConfigMap/base"}, {"ConfigMap/leaf"}}, client.waitCalls)
}

func TestInstall_Sequenced_TerminalGroupFailureFailsInstall(t *testing.T) {
	client := newRecordingKubeClient()
	...
	require.Error(t, err, "a terminal group's readiness failure must fail the install")
	assert.Equal(t, [][]string{{"ConfigMap/base"}, {"ConfigMap/leaf"}}, client.createCalls)
...
func TestInstall_Sequenced_AtomicRollback(t *testing.T) {
	client := newRecordingKubeClient()
	// Install-then-rollback needs cluster-like GET behavior: 404 before the
	// resources are created (conflict check), owned afterwards (rollback
	// deletion ownership verification).
...
// TestInstall_Sequenced_FailureCleanupIsSequenced locks beads ll8/0vr: the
// Sequenced flag must reach the STORED release record before deployment
// starts, so the rollback-on-failure uninstall, which re-reads the release
// from storage, tears the partial install down in reverse batch order with a
// delete-wait per batch, not as one unordered bulk delete.
func TestInstall_Sequenced_FailureCleanupIsSequenced(t *testing.T) {
	client := newRecordingKubeClient()
	client.buildRESTClient = statefulOwnedRESTClient(client, "test-install-release", "spaced")
	client.waitErrorOnCall = 2
	client.waitError = errors.New("batch two never became ready")
	...
	ch := buildChartWithTemplates([]*common.File{
		...
		makeConfigMapTemplate("templates/c.yaml", "c", map[string]string{
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "rollback-on-failure")
	assert.True(t, mustRelease(t, rel).Sequenced, "failed release must persist sequencing intent via the Sequenced flag")

	// a and b were created (c's batch never started; its manifests read as
	// already-deleted). Cleanup must delete in reverse group order, waiting
	// for each batch's deletion before the next.
	assert.Equal(t, [][]string{{"ConfigMap/b"}, {"ConfigMap/a"}}, client.deleteCalls)
	assert.Equal(t, [][]string{{"ConfigMap/b"}, {"ConfigMap/a"}}, client.deleteWaitCalls)
}

// TestInstall_Sequenced_TakeOwnershipAdoptsExisting locks bead 9ui (install):
// with --take-ownership, a resource that already exists in the cluster must be
// adopted via Update inside its plan batch, not blindly Created.
func TestInstall_Sequenced_TakeOwnershipAdoptsExisting(t *testing.T) {
	client := newRecordingKubeClient()
	client.buildRESTClient = statefulOwnedRESTClient(client, "test-install-release", "spaced")
	// database pre-exists in the cluster before the install begins.
	client.createdObjects["ConfigMap/database"] = true
	...
	// database's batch adopts (Update with the pre-existing object as current);
	// app's batch has nothing to adopt and is Created.
	require.Len(t, client.updateCalls, 1)
	assert.Equal(t, []string{"ConfigMap/database"}, client.updateCalls[0].current)
	assert.Equal(t, []string{"ConfigMap/database"}, client.updateCalls[0].target)
	assert.Equal(t, [][]string{{"ConfigMap/app"}}, client.createCalls)
}

// TestInstall_Sequenced_SameNameDifferentKindWaited locks beads xnf/p1m:
// readiness gating must never key on bare metadata.name. A ConfigMap and a
// Secret share the name "shared"; previously the leaf-suppression map keyed by
// name suppressed the wait for BOTH, including the depended-on ConfigMap.
func TestInstall_Sequenced_SameNameDifferentKindWaited(t *testing.T) {
	client := newRecordingKubeClient()
	install := newSequencedInstallAction(t, client)
	...
	ch := buildChartWithTemplates([]*common.File{
		makeConfigMapTemplate("templates/base.yaml", "shared", map[string]string{
			releaseutil.AnnotationResourceGroup: "base",
		}),
		releaseFile("templates/app.yaml", secretManifest),
	})

	mustRunInstall(t, install, ch)

	assert.Equal(t, [][]string{{"ConfigMap/shared"}, {"Secret/shared"}}, client.createCalls)
	assert.Equal(t, [][]string{{"ConfigMap/shared"}, {"Secret/shared"}}, client.waitCalls)
}
...
func TestInstall_Sequenced_CyclicDependency(t *testing.T) {
	...
	assert.Empty(t, client.createCalls)
	_, lastErr := install.cfg.Releases.Last(install.ReleaseName)
	assert.Error(t, lastErr, "cycle must be detected before any release record is stored")
}

func TestInstall_Sequenced_SequencedFlagStored(t *testing.T) {
	client := newRecordingKubeClient()
	install := newSequencedInstallAction(t, client)
	...
	require.NoError(t, err)
	storedRel := mustRelease(t, stored)
	assert.True(t, storedRel.Sequenced)
	assert.True(t, storedRel.IsSequenced())
	assert.Nil(t, storedRel.SequencingInfo)
}
...
func singleManifestBatch(m releaseutil.Manifest) sequence.Batch {
	return sequence.Batch{
		Groups: []sequence.Group{{Manifests: []releaseutil.Manifest{m}}},
		Wait:   true,
	}
}

func TestSequencedDeployment_ApplyBatch_Create_RespectsContextCancellation(t *testing.T) {
	...
	t.Run("context canceled before build", func(t *testing.T) {
		client := newRecordingKubeClient()
		buildCalled := false
		client.onBuild = func() { buildCalled = true }
		...
		err := sd.applyBatch(ctx, singleManifestBatch(makeTestManifest("cm", "chart/templates/cm.yaml", nil)))
...
func TestSequencedDeployment_ApplyBatch_Update_RespectsContextCancellation(t *testing.T) {
	newSequencedDeploymentForTest := func(client kube.Interface) *sequencedDeployment {
		t.Helper()
	...
	t.Run("context canceled after build before update", func(t *testing.T) {
		client := newRecordingKubeClient()
		ctx, cancel := context.WithCancel(context.Background())
		client.onBuild = cancel
	...
	t.Run("context canceled after update before wait", func(t *testing.T) {
		client := newRecordingKubeClient()
		ctx, cancel := context.WithCancel(context.Background())
		client.onUpdate = cancel
