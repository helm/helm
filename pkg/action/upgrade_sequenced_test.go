...
func TestUpgrade_Sequenced_Basic(t *testing.T) {
	...
	// Per HIP-0025 §Readiness: app is a leaf (no group depends on it), so its
	// readiness is not waited on; database is depended-on by app.
...
func TestUpgrade_Sequenced_NewResourceGroup(t *testing.T) {
	...
	// Per HIP-0025 §Readiness: app is a leaf (no group depends on it), so
	// its readiness is not waited on. database and queue have dependents.
	assert.Equal(t, [][]string{{"ConfigMap/database"}, {"ConfigMap/queue"}}, client.waitCalls)
}
...
func TestUpgrade_Sequenced_RemovedResourceGroup(t *testing.T) {
	...
	// Per HIP-0025 §Readiness: app is a leaf in the upgraded chart's DAG
	// (no group depends on it), so its readiness is not waited on.
