...
func TestUpgrade_Sequenced_FailureRollback(t *testing.T) {
	...
	rel, err := upgrade.Run("failure-rollback", upgradedChart, map[string]interface{}{})
	require.Error(t, err)
...
func TestUpgrade_Sequenced_CleanupOnFail(t *testing.T) {
	...
	rel, err := upgrade.Run("cleanup-on-fail", upgradedChart, map[string]interface{}{})
	require.Error(t, err)
...
func mustRunUpgrade(t *testing.T, upgrade *Upgrade, name string, ch *chart.Chart) ri.Releaser {
	t.Helper()
	rel, err := upgrade.Run(name, ch, map[string]interface{}{})
