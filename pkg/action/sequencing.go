...
type sequencedDeployment struct {
	...
	// leafResourceNames is the set of resource names that belong to
	// resource-groups with no dependents (no other group declares them via
	// helm.sh/depends-on/resource-groups). Per HIP-0025 §Readiness, these
	// resources are still applied but their readiness is not waited on.
	// Populated per chart-level in deployResourceGroupBatches.
	leafResourceNames map[string]bool
}
...
func (s *sequencedDeployment) deployResourceGroupBatches(ctx context.Context, manifests []releaseutil.Manifest) error {
	...
	// Leaf-resource detection is scoped per chart-level. Reset on entry so
	// a parent-chart's leaves don't leak into a subchart's wait decisions.
	s.leafResourceNames = nil
	...
	if len(result.Groups) > 0 {
		...
		// Per HIP-0025 §Readiness: readiness of a resource with no dependents
		// must be ignored. Identify resources whose group is not depended on
		// by any other group so waitForResources can skip them.
		s.leafResourceNames = collectLeafResourceNames(result)
...
// collectLeafResourceNames returns the set of manifest names whose
// resource-group has no dependents within this chart-level's DAG. Per
// HIP-0025 §Readiness, these resources are applied but their readiness is
// not waited on.
func collectLeafResourceNames(result releaseutil.ResourceGroupResult) map[string]bool {
	groupHasDependents := make(map[string]bool)
	for _, deps := range result.GroupDeps {
		for _, dep := range deps {
			groupHasDependents[dep] = true
		}
	}
	leaves := make(map[string]bool)
	for groupName, groupManifests := range result.Groups {
		if groupHasDependents[groupName] {
			continue
		}
		for _, m := range groupManifests {
			if m.Head == nil || m.Head.Metadata == nil || m.Head.Metadata.Name == "" {
				continue
			}
			leaves[m.Head.Metadata.Name] = true
		}
	}
	return leaves
}
...
func (s *sequencedDeployment) waitForResources(resources kube.ResourceList, manifests []releaseutil.Manifest) error {
	// Per HIP-0025 §Readiness ("A resources readiness is checked if there is
	// a resource that depends on it as per the sequencing DAG, otherwise the
	// checks are ignored."): filter out resources whose group is a leaf in
	// the resource-group DAG. They are applied normally but their readiness
	// is not waited on. leafResourceNames may be nil/empty when the chart has
	// no sequenced groups, in which case no filtering occurs.
	var resourcesToWait kube.ResourceList
	if len(s.leafResourceNames) == 0 {
		resourcesToWait = resources
	} else {
		resourcesToWait = make(kube.ResourceList, 0, len(resources))
		for _, r := range resources {
			if s.leafResourceNames[r.Name] {
				continue
			}
			resourcesToWait = append(resourcesToWait, r)
		}
	}
	if len(resourcesToWait) == 0 {
		return nil
	}
	...
	if s.waitForJobs {
		return waiter.WaitWithJobs(resourcesToWait, waitTimeout)
	}
	return waiter.Wait(resourcesToWait, waitTimeout)
}
