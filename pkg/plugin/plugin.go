...
// - From the PlatformCommand where OS matches the current platform and Arch is empty
// - From the PlatformCommand where OS and Arch are both empty
// - Return nil, nil
func getPlatformCommand(cmds []PlatformCommand) ([]string, []string) {
	...
	for _, c := range cmds {
		...
		if eq(c.OperatingSystem, runtime.GOOS) || (len(c.OperatingSystem) == 0 && !found) {
...
// PrepareCommands takes a []Plugin.PlatformCommand with a legacy fallback
