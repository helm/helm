...
func validatePluginData(plug *Plugin, filepath string) error {
	...
	if len(plug.Metadata.PlatformCommand) > 0 && len(plug.Metadata.Command) > 0 {
		slog.Warn("both 'platformCommand' and 'command' are set (this will become an error in a future Helm version)", slog.String("pluginFilePath", filepath))
	}

	if len(plug.Metadata.PlatformHooks) > 0 && len(plug.Metadata.Hooks) > 0 {
		slog.Warn("both 'platformHook' and 'hook' are set (this will become an error in a future Helm version)", slog.String("pluginFilePath", filepath))
	}
