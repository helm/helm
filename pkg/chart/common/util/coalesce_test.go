...
func TestCoalesceValuesWarnings(t *testing.T) {
	...
	t.Logf("vals: %v", vals)
	assert.Contains(t, warnings, "warning: skipped value for level1.level2.level3.boat: not a YAML mapping (a map or object). Chart default is a scalar value but your values provide a nested mapping under this key.")
	assert.Contains(t, warnings, "warning: ignoring chart default non-mapping value at level1.level2.level3.spear.tip: user-provided value is a YAML mapping (a map or object), but the chart default is not (true). Use a non-mapping (scalar) value for this key in your values file, or check that the chart expects a mapping here.")
	assert.Contains(t, warnings, "warning: cannot overwrite value at level1.level2.level3.spear.sail: chart default is a YAML mapping (a map or object), but user-provided value is not (map[cotton:true]). Provide a YAML mapping (nested key-value pairs) for this key in your values file, or ensure the chart expects a non-mapping value here.")
}
