...
func coalesceGlobals(printf printFn, dest, src map[string]any, prefix string, _ bool) {
	...
	if destglob, ok := dest[common.GlobalKey]; !ok {
		...
		printf("warning: skipping globals because destination %s is not a YAML mapping (a map or object).", common.GlobalKey)
		return
	}
	...
	if srcglob, ok := src[common.GlobalKey]; !ok {
		...
		printf("warning: skipping globals because source %s is not a YAML mapping (a map or object).", common.GlobalKey)
		return
	}
	...
	for key, val := range sg {
		if istable(val) {
			...
			printf("key %s is a YAML mapping (a map or object) in the destination. Skipping", key)
		} else {
		...
		if value, ok := v[key]; ok {
			if value == nil && !merge {
				...
				if !ok {
					...
					// the warning
					if val != nil {
						printf("warning: skipped value for %s.%s: not a YAML mapping (a map or object). Chart default is a scalar value but your values provide a nested mapping under this key.", subPrefix, key)
					}
				} else {
	...
	for key, val := range src {
		...
		if dv, ok := dst[key]; ok && !merge && dv == nil && srcOriginalNonNil[key] {
			...
			if istable(dv) {
				...
				printf("warning: cannot overwrite value at %s: chart default is a YAML mapping (a map or object), but user-provided value is not (%v). Provide a YAML mapping (nested key-value pairs) for this key in your values file, or ensure the chart expects a non-mapping value here.", fullkey, val)
			}
		} else if istable(dv) && val != nil {
			printf("warning: ignoring chart default non-mapping value at %s: user-provided value is a YAML mapping (a map or object), but the chart default is not (%v). Use a non-mapping (scalar) value for this key in your values file, or check that the chart expects a mapping here.", fullkey, val)
		}
	}
	return dst
}
