...
func init() {
	if v, err := strconv.Atoi(os.Getenv("DEFAULT_HTTP_TIMEOUT")); err == nil {
		DefaultHTTPTimeout = v
	}
}
...
var httpProvider = Provider{
	...
	New: func(options ...Option) (Getter, error) {
		newOptions := []Option{WithTimeout(time.Second * time.Duration(DefaultHTTPTimeout))}
