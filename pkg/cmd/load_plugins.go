...
// as it finds them.
func loadCLIPlugins(baseCmd *cobra.Command, logger *slog.Logger, out io.Writer) {
	...
	for _, plug := range found {
		...
		if (err == nil &&
			...
			loadCompletionForPlugin(logger, c, plug)
		}
	}
}
...
func loadCompletionForPlugin(logger *slog.Logger, pluginCmd *cobra.Command, plug plugin.Plugin) {
	...
	addPluginCommands(logger, plug, pluginCmd, cmds)
}
...
func addPluginCommands(logger *slog.Logger, plug plugin.Plugin, baseCmd *cobra.Command, cmds *pluginCommand) {
	if cmds == nil {
		return
	}
	...
	for _, cmd := range cmds.Commands {
		...
		addPluginCommands(logger, plug, subCmd, &cmd)
	}
}
