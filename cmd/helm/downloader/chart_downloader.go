...
const (
	...
	// VerifyIfPossible will attempt a verification, but will not stop processing
...
func (c *ChartDownloader) DownloadTo(ref string, dest string) (*provenance.Verification, error) {
	...
	var ver *provenance.Verification
	if c.Verify > VerifyNever {
		...
		if err != nil {
			...
			fmt.Fprintf(c.Out, "WARNING: Verification not found for %s: %s", ref, err)
		}
