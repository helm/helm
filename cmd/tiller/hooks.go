...
// returned.
func sortHooks(files map[string]string, apis versionSet) ([]*release.Hook, map[string]string, error) {
	hs := []*release.Hook{}
	...
	for n, c := range files {
		...
		if sh.Version != "" && !apis.Has(sh.Version) {
			return hs, generic, fmt.Errorf("apiVersion %q is not available", sh.Version)
		}
