...
mapfile -t failed_license_header < <(find_files | xargs grep -L 'Licensed under the Apache License, Version 2.0 (the "License")' || true)
...
# Use "|| :" to ignore the error code when grep returns empty
mapfile -t failed_copyright_header < <(find_files | xargs grep -L 'Copyright The Helm Authors.' || true)
